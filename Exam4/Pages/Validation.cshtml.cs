...
namespace Exam4.Pages
{
    public class ValidationModel : PageModel
    {
        ...
        public IActionResult OnGet(string? token)
        {
            ...
            {
                Console.WriteLine("?? Email already validated");
                IsSuccess = true;
            ...
            // Mark as validated
            Console.WriteLine("?? Marking email as validated");
            credentials.MarkAsValidated();
            _credentialsService.UpdateCredentials(credentials);

            Console.WriteLine($"? Email validation completed for: {credentials.Email}");
